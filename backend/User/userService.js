...
const signup = async (req, res, next) => {
  const { nom, prenom, mail, numero,role, password } = req.body;
  const { pdp, docVerif } = req.files;
  let existingUser;
try{
...
const user = new UserModel ({
      ...
      pdp: (pdp && pdp[0].filename) || null,
      docVerif: (docVerif && docVerif.map(file => file.filename)) || []
    });
